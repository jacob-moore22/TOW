...
| **Purpose** | Solve Ax=b using SVD back-substitution given the decomposition from svdcmp. |
| **Status** | `not_started` |
| **Complexity** | `low` |
| **Fortran LOC** | 23 |
| **Subroutine** | `SVBKSB` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/02_linear_algebra/svbksb/svbksb.f` (23 lines)
- **Driver/demo:** `fortran/02_linear_algebra/svbksb/svbksb.dem`
- **Data files:** `MATRX1.DAT`
- **Target:** `matarized/02_linear_algebra/svbksb/`
...
|------|-------------|-------|------|-----------|-------|
| `B` | `REAL` | MP | parameter (input) | `DFMatrixKokkos<double>(MP)` |  |
...
| `TMP` | `REAL` | NMAX | local | `DFMatrixKokkos<double>(NMAX)` |  |
...
```cpp
inline void svbksb(DFMatrixKokkos<double>& u, DFMatrixKokkos<double>& w, DFMatrixKokkos<double>& v, int m, int n, int mp, int np, DFMatrixKokkos<double>& b, DFMatrixKokkos<double>& x)
```
...
2. **Translate routine** -- convert `SVBKSB` to a C++ function as a `.hpp header`
...
project(svbksb_matar_parallel CXX)
...
# --- Build the SVBKSB example ---
add_executable(svbksb main.cpp)
target_link_libraries(svbksb matar_lib)
target_include_directories(svbksb PRIVATE
...
```bash
cd fortran/02_linear_algebra/svbksb
...
```bash
cd matarized/02_linear_algebra/svbksb
mkdir -p build && cd build
cmake .. && make
./svbksb > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/02_linear_algebra/svbksb/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./svbksb > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./svbksb > omp4_output.txt 2>&1
