...
| **Purpose** | Construct 2D cubic spline coefficients from tabulated function values. |
| **Status** | `not_started` |
| **Complexity** | `low` |
| **Fortran LOC** | 15 |
| **Subroutine** | `SPLIE2` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/03_interpolation/splie2/splie2.f` (15 lines)
- **Driver/demo:** `fortran/03_interpolation/splie2/splie2.dem`
- **Target:** `matarized/03_interpolation/splie2/`
...
```cpp
inline void splie2(DFMatrixKokkos<double>& x1a, DFMatrixKokkos<double>& x2a, DFMatrixKokkos<double>& ya, int m, int n, DFMatrixKokkos<double>& y2a)
```
...
2. **Translate routine** -- convert `SPLIE2` to a C++ function as a `.hpp header`
...
project(splie2_matar_parallel CXX)
...
# --- Build the SPLIE2 example ---
add_executable(splie2 main.cpp)
target_link_libraries(splie2 matar_lib)
target_include_directories(splie2 PRIVATE
...
```bash
cd fortran/03_interpolation/splie2
...
```bash
cd matarized/03_interpolation/splie2
mkdir -p build && cd build
cmake .. && make
./splie2 > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/03_interpolation/splie2/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./splie2 > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./splie2 > omp4_output.txt 2>&1
