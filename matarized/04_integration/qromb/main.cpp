...
int main(int argc, char* argv[])
{
    MATAR_INITIALIZE(argc, argv);
    {
        ...
        std::printf("Romberg integration (qromb) tests\n\n");
        ...
        double r1 = qromb(f_x2,  0.0, 1.0);
        double r2 = qromb(f_sin, 0.0, pi);
        double r3 = qromb(f_exp, 0.0, 1.0);
        ...
        double r4 = qromb(f_sin, 0.0, pi, 1.0e-10);
        std::printf("\nsin(x) on [0,pi] (eps=1e-10):\n");
        std::printf("  Result   = %.14f\n", r4);
        std::printf("  Expected = %.14f\n", e2);
        std::printf("  Error    = %.2e\n", std::fabs(r4 - e2));

        bool pass = (std::fabs(r1 - e1) < 1e-6) &&
                    (std::fabs(r2 - e2) < 1e-6) &&
                    (std::fabs(r3 - e3) < 1e-6) &&
                    (std::fabs(r4 - e2) < 1e-10);
        std::printf("\nTest %s\n", pass ? "PASSED" : "FAILED");
    }
    MATAR_FINALIZE();
    return 0;
