...
int main(int argc, char* argv[])
{
    MATAR_INITIALIZE(argc, argv);
    {
        ...
        constexpr double ref_I0_1 = 1.2660658777520084;

        std::printf("Modified Bessel I0(x) — MATAR parallel evaluation:\n");
        std::printf("%12s %20s\n", "x", "I0(x)");
        for (int i = 0; i < NTEST; i++)
            std::printf("%12.4f %20.12f\n", test_x[i], result.host(i));

        double err = std::fabs(result.host(2) - ref_I0_1);
        std::printf("\nI0(1.0) = %.16f  (ref %.16f)\n", result.host(2), ref_I0_1);
        std::printf("Error: %.2e\n", err);
