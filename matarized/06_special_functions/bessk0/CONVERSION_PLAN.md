...
dependencies: [bessi0]
reverse_dependencies: [bessk]
---

# BESSK0 -- Special Functions
...
| **Purpose** | Modified Bessel function K0(x) using polynomial approximation. |
| **Status** | `not_started` |
| **Complexity** | `low` |
| **Fortran LOC** | 19 |
| **Subroutine** | `BESSK0` (function) |

## 2. Source Files

- **Fortran source:** `fortran/06_special_functions/bessk0/bessk0.f` (19 lines)
- **Driver/demo:** `fortran/06_special_functions/bessk0/bessk0.dem`
- **Data files:** `FNCVAL.DAT`
- **Target:** `matarized/06_special_functions/bessk0/`
...
2. **Translate routine** -- convert `BESSK0` to a C++ function as a `.hpp header`
...
project(bessk0_matar_parallel CXX)
...
# --- Build the BESSK0 example ---
add_executable(bessk0 main.cpp)
target_link_libraries(bessk0 matar_lib)
target_include_directories(bessk0 PRIVATE
...
```bash
cd fortran/06_special_functions/bessk0
...
```bash
cd matarized/06_special_functions/bessk0
mkdir -p build && cd build
cmake .. && make
./bessk0 > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/06_special_functions/bessk0/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./bessk0 > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./bessk0 > omp4_output.txt 2>&1
...
| Field | Value |
|-------|-------|
| **Conversion order** | 77 of 202 |
| **Priority score** | 1 (reverse dependency count) |
| **Estimated effort** | low (19 Fortran LOC, 1 dependencies) |
