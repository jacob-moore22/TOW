...
| **Purpose** | Modified Bessel function K1(x) using polynomial approximation. |
| **Status** | `not_started` |
| **Complexity** | `low` |
| **Fortran LOC** | 19 |
| **Subroutine** | `BESSK1` (function) |

## 2. Source Files

- **Fortran source:** `fortran/06_special_functions/bessk1/bessk1.f` (19 lines)
- **Driver/demo:** `fortran/06_special_functions/bessk1/bessk1.dem`
- **Data files:** `FNCVAL.DAT`
- **Target:** `matarized/06_special_functions/bessk1/`
...
2. **Translate routine** -- convert `BESSK1` to a C++ function as a `.hpp header`
...
project(bessk1_matar_parallel CXX)
...
# --- Build the BESSK1 example ---
add_executable(bessk1 main.cpp)
target_link_libraries(bessk1 matar_lib)
target_include_directories(bessk1 PRIVATE
...
```bash
cd fortran/06_special_functions/bessk1
...
```bash
cd matarized/06_special_functions/bessk1
mkdir -p build && cd build
cmake .. && make
./bessk1 > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/06_special_functions/bessk1/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./bessk1 > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./bessk1 > omp4_output.txt 2>&1
