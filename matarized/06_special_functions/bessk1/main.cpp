...
int main(int argc, char* argv[])
{
    MATAR_INITIALIZE(argc, argv);
    {
        ...
        constexpr double ref_K1_1 = 0.6019072301972346;

        std::printf("Modified Bessel K1(x) — MATAR parallel evaluation:\n");
        std::printf("%12s %20s\n", "x", "K1(x)");
        for (int i = 0; i < NTEST; i++)
            std::printf("%12.4f %20.12f\n", test_x[i], result.host(i));

        double err = std::fabs(result.host(2) - ref_K1_1);
        std::printf("\nK1(1.0) = %.16f  (ref %.16f)\n", result.host(2), ref_K1_1);
        std::printf("Error: %.2e\n", err);
