...
| **Purpose** | General complete elliptic integral. |
| **Status** | `not_started` |
| **Complexity** | `medium` |
| **Fortran LOC** | 40 |
| **Subroutine** | `CEL` (function) |

## 2. Source Files

- **Fortran source:** `fortran/06_special_functions/cel/cel.f` (40 lines)
- **Driver/demo:** `fortran/06_special_functions/cel/cel.dem`
- **Target:** `matarized/06_special_functions/cel/`
...
| `PIO2` | `REAL` | (scalar) | constant | `constexpr double PIO2 = 1.5707963268;` | constant = 1.5707963268 |
...
```cpp
inline double cel(double qqc, double pp, double aa, double bb)
```
...
2. **Translate routine** -- convert `CEL` to a C++ function as a `.cpp with main()`
...
project(cel_matar_parallel CXX)
...
# --- Build the CEL example ---
add_executable(cel main.cpp)
target_link_libraries(cel matar_lib)
target_include_directories(cel PRIVATE
...
```bash
cd fortran/06_special_functions/cel
...
```bash
cd matarized/06_special_functions/cel
mkdir -p build && cd build
cmake .. && make
./cel > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/06_special_functions/cel/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./cel > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./cel > omp4_output.txt 2>&1
...
| Field | Value |
|-------|-------|
| **Conversion order** | 160 of 202 |
| **Priority score** | 0 (reverse dependency count) |
| **Estimated effort** | medium (40 Fortran LOC, 3 dependencies) |
