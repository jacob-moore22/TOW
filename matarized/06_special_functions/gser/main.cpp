...
int main(int argc, char* argv[])
{
    MATAR_INITIALIZE(argc, argv);
    {
        ...
        double test_a[NTEST] = {0.5, 1.0, 2.0, 5.0};
        double test_x[NTEST] = {0.5, 1.0, 1.5, 3.0};
        ...
        std::printf("Incomplete gamma series P(a,x) via gser:\n");
        ...
        double expected = 1.0 - exp(-1.0);
        double err = fabs(result.host(1) - expected);
        std::printf("\ngser(1.0, 1.0) = %.10f  expected = %.10f  err = %.2e\n",
                    result.host(1), expected, err);
