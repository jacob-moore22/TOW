...
| **Purpose** | Minimize in N-D using Broyden-Fletcher-Goldfarb-Shanno (BFGS) quasi-Newton. |
| **Status** | `not_started` |
| **Complexity** | `high` |
| **Fortran LOC** | 60 |
| **Subroutine** | `DFPMIN` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/10_optimization/dfpmin/dfpmin.f` (60 lines)
- **Driver/demo:** `fortran/10_optimization/dfpmin/dfpmin.dem`
- **Target:** `matarized/10_optimization/dfpmin/`
...
|------|-------------|-------|------|-----------|-------|
| `DG` | `REAL` | NMAX | local | `DFMatrixKokkos<double>(NMAX)` |  |
...
| `HDG` | `REAL` | NMAX | local | `DFMatrixKokkos<double>(NMAX)` |  |
| `HESSIN` | `REAL` | NMAX, NMAX | local | `DFMatrixKokkos<double>(NMAX, NMAX)` |  |
...
```cpp
inline void dfpmin(DFMatrixKokkos<double>& p, int n, double ftol, int iter, double fret)
```
...
2. **Translate routine** -- convert `DFPMIN` to a C++ function as a `.cpp with main()`
...
project(dfpmin_matar_parallel CXX)
...
# --- Build the DFPMIN example ---
add_executable(dfpmin main.cpp)
target_link_libraries(dfpmin matar_lib)
target_include_directories(dfpmin PRIVATE
...
```bash
cd fortran/10_optimization/dfpmin
...
```bash
cd matarized/10_optimization/dfpmin
mkdir -p build && cd build
cmake .. && make
./dfpmin > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/10_optimization/dfpmin/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./dfpmin > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./dfpmin > omp4_output.txt 2>&1
...
| Field | Value |
|-------|-------|
| **Conversion order** | 189 of 202 |
| **Priority score** | 0 (reverse dependency count) |
| **Estimated effort** | high (60 Fortran LOC, 6 dependencies) |
