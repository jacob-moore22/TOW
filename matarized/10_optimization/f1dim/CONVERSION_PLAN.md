...
| **Purpose** | Utility for linmin: project an N-D function onto a line. |
| **Status** | `not_started` |
| **Complexity** | `low` |
| **Fortran LOC** | 11 |
| **Subroutine** | `F1DIM` (function) |

## 2. Source Files

- **Fortran source:** `fortran/10_optimization/f1dim/f1dim.f` (11 lines)
- **Driver/demo:** `fortran/10_optimization/f1dim/f1dim.dem`
- **Target:** `matarized/10_optimization/f1dim/`
...
2. **Translate routine** -- convert `F1DIM` to a C++ function as a `.hpp header`
...
project(f1dim_matar_parallel CXX)
...
# --- Build the F1DIM example ---
add_executable(f1dim main.cpp)
target_link_libraries(f1dim matar_lib)
target_include_directories(f1dim PRIVATE
...
```bash
cd fortran/10_optimization/f1dim
...
```bash
cd matarized/10_optimization/f1dim
mkdir -p build && cd build
cmake .. && make
./f1dim > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/10_optimization/f1dim/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./f1dim > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./f1dim > omp4_output.txt 2>&1
