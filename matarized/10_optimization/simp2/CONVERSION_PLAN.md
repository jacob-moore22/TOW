...
| **Purpose** | Simplex utility: locate the pivot row. |
| **Status** | `library_only` |
| **Complexity** | `medium` |
| **Fortran LOC** | 32 |
| **Subroutine** | `SIMP2` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/10_optimization/simp2/simp2.f` (32 lines)
- **Driver/demo:** _(none -- library-only or program-in-.f)_
- **Target:** `matarized/10_optimization/simp2/`
...
```cpp
inline void simp2(DFMatrixKokkos<double>& a, int m, int n, int mp, int np, DFMatrixKokkos<double>& l2, int nl2, int ip, int kp, double q1)
```
...
2. **Translate routine** -- convert `SIMP2` to a C++ function as a `.hpp header`
...
project(simp2_matar_parallel CXX)
...
# --- Build the SIMP2 example ---
add_executable(simp2 main.cpp)
target_link_libraries(simp2 matar_lib)
...
```bash
cd matarized/10_optimization/simp2
mkdir -p build && cd build
cmake .. && make
./simp2 > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/10_optimization/simp2/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./simp2 > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./simp2 > omp4_output.txt 2>&1
...
| Field | Value |
|-------|-------|
| **Conversion order** | 72 of 202 |
| **Priority score** | 1 (reverse dependency count) |
| **Estimated effort** | medium (32 Fortran LOC, 0 dependencies) |
