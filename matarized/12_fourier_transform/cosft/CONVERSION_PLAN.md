...
| **Purpose** | Discrete cosine transform using realft. |
| **Status** | `not_started` |
| **Complexity** | `medium` |
| **Fortran LOC** | 46 |
| **Subroutine** | `COSFT` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/12_fourier_transform/cosft/cosft.f` (46 lines)
- **Driver/demo:** `fortran/12_fourier_transform/cosft/cosft.dem`
- **Target:** `matarized/12_fourier_transform/cosft/`
...
- **Notes:** Accumulates: EVEN, ODD, SUM, WI, WR Array write(s) not indexed by loop variable: Y. Verify thread safety.

### K2: DO 12  J=4,N, step 2

- **Thread safety:** `unsafe_review`
- **Recommended macro:** `DO_REDUCE_SUM`
- **Notes:** Accumulates: EVEN, ODD, SUM, WI, WR Array write(s) not indexed by loop variable: Y. Verify thread safety.

### K3: DO 13  I=3,N-1, step 2

- **Thread safety:** `unsafe_review`
- **Recommended macro:** `DO_REDUCE_SUM`
- **Notes:** Accumulates: EVEN, ODD, SUM, WI, WR Array write(s) not indexed by loop variable: Y. Verify thread safety.

### K4: DO 14  I=3,N-1, step 2
...
2. **Translate routine** -- convert `COSFT` to a C++ function as a `.cpp with main()`
...
project(cosft_matar_parallel CXX)
...
# --- Build the COSFT example ---
add_executable(cosft main.cpp)
target_link_libraries(cosft matar_lib)
target_include_directories(cosft PRIVATE
...
```bash
cd fortran/12_fourier_transform/cosft
...
```bash
cd matarized/12_fourier_transform/cosft
mkdir -p build && cd build
cmake .. && make
./cosft > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/12_fourier_transform/cosft/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./cosft > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./cosft > omp4_output.txt 2>&1
...
| Field | Value |
|-------|-------|
| **Conversion order** | 147 of 202 |
| **Priority score** | 0 (reverse dependency count) |
| **Estimated effort** | medium (46 Fortran LOC, 2 dependencies) |
