...
| **Purpose** | F-test for comparing two variances. |
| **Status** | `not_started` |
| **Complexity** | `high` |
| **Fortran LOC** | 18 |
| **Subroutine** | `FTEST` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/14_statistics/ftest/ftest.f` (18 lines)
- **Driver/demo:** `fortran/14_statistics/ftest/ftest.dem`
- **Target:** `matarized/14_statistics/ftest/`
...
```cpp
inline void ftest(DFMatrixKokkos<double>& data1, int n1, DFMatrixKokkos<double>& data2, int n2, double f, double prob)
```
...
2. **Translate routine** -- convert `FTEST` to a C++ function as a `.cpp with main()`
...
project(ftest_matar_parallel CXX)
...
# --- Build the FTEST example ---
add_executable(ftest main.cpp)
target_link_libraries(ftest matar_lib)
target_include_directories(ftest PRIVATE
...
```bash
cd fortran/14_statistics/ftest
...
```bash
cd matarized/14_statistics/ftest
mkdir -p build && cd build
cmake .. && make
./ftest > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/14_statistics/ftest/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./ftest > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./ftest > omp4_output.txt 2>&1
...
| Field | Value |
|-------|-------|
| **Conversion order** | 191 of 202 |
| **Priority score** | 0 (reverse dependency count) |
| **Estimated effort** | high (18 Fortran LOC, 6 dependencies) |
