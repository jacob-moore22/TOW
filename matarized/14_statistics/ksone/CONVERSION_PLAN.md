...
status: not_started
complexity: high
conversion_order: 201
priority: 0
tags: [statistics, hypothesis-testing, cross-chapter]
dependencies: [erf, gammln, gammp, gcf, gser, gasdev, ran1, sort, probks]
...
| **Purpose** | Kolmogorov-Smirnov test: compare data to a known distribution function. |
| **Status** | `not_started` |
| **Complexity** | `high` |
| **Fortran LOC** | 17 |
| **Subroutine** | `KSONE` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/14_statistics/ksone/ksone.f` (17 lines)
- **Driver/demo:** `fortran/14_statistics/ksone/ksone.dem`
- **Target:** `matarized/14_statistics/ksone/`
...
```cpp
inline void ksone(DFMatrixKokkos<double>& data, int n, double func, double d, double prob)
```
...
2. **Translate routine** -- convert `KSONE` to a C++ function as a `.cpp with main()`
...
project(ksone_matar_parallel CXX)
...
# --- Build the KSONE example ---
add_executable(ksone main.cpp)
target_link_libraries(ksone matar_lib)
target_include_directories(ksone PRIVATE
    ${SPECIALFUNCTIONS_DIR}/erf
...
```bash
cd fortran/14_statistics/ksone
...
```bash
cd matarized/14_statistics/ksone
mkdir -p build && cd build
cmake .. && make
./ksone > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/14_statistics/ksone/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./ksone > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./ksone > omp4_output.txt 2>&1
...
| Field | Value |
|-------|-------|
| **Conversion order** | 201 of 202 |
| **Priority score** | 0 (reverse dependency count) |
| **Estimated effort** | high (17 Fortran LOC, 9 dependencies) |
| **Prerequisite conversions** | `erf`, `gammln`, `gammp`, `gcf`, `gser`, `gasdev`, `ran1`, `sort`, `probks` |
| **Tags** | `statistics`, `hypothesis-testing`, `cross-chapter` |
