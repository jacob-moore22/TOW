...
| **Purpose** | Student's t-test for difference of means (equal variances assumed). |
| **Status** | `not_started` |
| **Complexity** | `high` |
| **Fortran LOC** | 11 |
| **Subroutine** | `TTEST` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/14_statistics/ttest/ttest.f` (11 lines)
- **Driver/demo:** `fortran/14_statistics/ttest/ttest.dem`
- **Target:** `matarized/14_statistics/ttest/`
...
```cpp
inline void ttest(DFMatrixKokkos<double>& data1, int n1, DFMatrixKokkos<double>& data2, int n2, double t, double prob)
```
...
2. **Translate routine** -- convert `TTEST` to a C++ function as a `.cpp with main()`
...
project(ttest_matar_parallel CXX)
...
# --- Build the TTEST example ---
add_executable(ttest main.cpp)
target_link_libraries(ttest matar_lib)
...
```bash
cd fortran/14_statistics/ttest
...
```bash
cd matarized/14_statistics/ttest
mkdir -p build && cd build
cmake .. && make
./ttest > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/14_statistics/ttest/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./ttest > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./ttest > omp4_output.txt 2>&1
...
| Field | Value |
|-------|-------|
| **Conversion order** | 196 of 202 |
| **Priority score** | 0 (reverse dependency count) |
| **Estimated effort** | high (11 Fortran LOC, 6 dependencies) |
