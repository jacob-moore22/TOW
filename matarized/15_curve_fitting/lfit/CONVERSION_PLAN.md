...
status: not_started
complexity: high
conversion_order: 192
priority: 0
tags: [curve-fitting, regression, least-squares, cross-chapter]
dependencies: [gaussj, beta, gammln, gasdev, ran1, covsrt]
...
| **Purpose** | General linear least-squares fit to a set of basis functions. |
| **Status** | `not_started` |
| **Complexity** | `high` |
| **Fortran LOC** | 66 |
| **Subroutine** | `LFIT` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/15_curve_fitting/lfit/lfit.f` (66 lines)
- **Driver/demo:** `fortran/15_curve_fitting/lfit/lfit.dem`
- **Target:** `matarized/15_curve_fitting/lfit/`
...
| `COVAR` | `REAL` | NCVM, NCVM | parameter (input) | `DFMatrixKokkos<double>(NCVM, NCVM)` |  |
...
| `NCVM` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
...
- **Notes:** Accumulates: CHISQ, IHIT, KK, SUM
...
```cpp
inline void lfit(DFMatrixKokkos<double>& x, DFMatrixKokkos<double>& y, DFMatrixKokkos<double>& sig, int ndata, DFMatrixKokkos<double>& a, int ma, DFMatrixKokkos<double>& lista, int mfit, DFMatrixKokkos<double>& covar, int ncvm, double chisq, double funcs)
```
...
2. **Translate routine** -- convert `LFIT` to a C++ function as a `.cpp with main()`
...
project(lfit_matar_parallel CXX)
...
# --- Build the LFIT example ---
add_executable(lfit main.cpp)
target_link_libraries(lfit matar_lib)
target_include_directories(lfit PRIVATE
...
```bash
cd fortran/15_curve_fitting/lfit
...
```bash
cd matarized/15_curve_fitting/lfit
mkdir -p build && cd build
cmake .. && make
./lfit > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/15_curve_fitting/lfit/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./lfit > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./lfit > omp4_output.txt 2>&1
...
| Field | Value |
|-------|-------|
| **Conversion order** | 192 of 202 |
| **Priority score** | 0 (reverse dependency count) |
| **Estimated effort** | high (66 Fortran LOC, 6 dependencies) |
| **Prerequisite conversions** | `gaussj`, `beta`, `gammln`, `gasdev`, `ran1`, `covsrt` |
| **Tags** | `curve-fitting`, `regression`, `least-squares`, `cross-chapter` |
