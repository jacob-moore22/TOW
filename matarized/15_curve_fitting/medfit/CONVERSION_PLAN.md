...
status: not_started
complexity: high
conversion_order: 202
priority: 0
tags: [curve-fitting, regression, least-squares, cross-chapter]
dependencies: [gammln, gammq, gcf, gser, gasdev, ran1, sort, fit, rofunc]
reverse_dependencies: []
---

# MEDFIT -- Curve Fitting
...
| **Purpose** | Robust least-absolute-deviation (L1-norm) straight line fit. |
| **Status** | `not_started` |
| **Complexity** | `high` |
| **Fortran LOC** | 58 |
| **Subroutine** | `MEDFIT` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/15_curve_fitting/medfit/medfit.f` (58 lines)
- **Driver/demo:** `fortran/15_curve_fitting/medfit/medfit.dem`
- **Target:** `matarized/15_curve_fitting/medfit/`
...
| `ABDEV` | `REAL` | (scalar) | parameter (input) | `double` |  |
...
- **Notes:** Accumulates: CHISQ, SX, SXX, SXY, SY
...
```cpp
inline void medfit(DFMatrixKokkos<double>& x, DFMatrixKokkos<double>& y, int ndata, double a, double b, double abdev)
```
...
2. **Translate routine** -- convert `MEDFIT` to a C++ function as a `.cpp with main()`
...
project(medfit_matar_parallel CXX)
...
# --- Build the MEDFIT example ---
add_executable(medfit main.cpp)
target_link_libraries(medfit matar_lib)
target_include_directories(medfit PRIVATE
    ...
    ${SORTING_DIR}/sort
    ${CURVEFITTING_DIR}/fit
    ${CURVEFITTING_DIR}/rofunc
)
```
...
```bash
cd fortran/15_curve_fitting/medfit
...
```bash
cd matarized/15_curve_fitting/medfit
mkdir -p build && cd build
cmake .. && make
./medfit > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/15_curve_fitting/medfit/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./medfit > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./medfit > omp4_output.txt 2>&1
...
| Field | Value |
|-------|-------|
| **Conversion order** | 202 of 202 |
| **Priority score** | 0 (reverse dependency count) |
| **Estimated effort** | high (58 Fortran LOC, 9 dependencies) |
| **Prerequisite conversions** | `gammln`, `gammq`, `gcf`, `gser`, `gasdev`, `ran1`, `sort`, `fit`, `rofunc` |
| **Tags** | `curve-fitting`, `regression`, `least-squares`, `cross-chapter` |
