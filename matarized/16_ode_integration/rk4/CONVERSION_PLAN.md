...
reverse_dependencies: [odeint, rkdumb, rkqc, shoot, shootf]
---
...
| **Purpose** | Fourth-order Runge-Kutta single-step ODE integrator. |
| **Status** | `not_started` |
| **Complexity** | `medium` |
| **Fortran LOC** | 25 |
| **Subroutine** | `RK4` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/16_ode_integration/rk4/rk4.f` (25 lines)
- **Driver/demo:** `fortran/16_ode_integration/rk4/rk4.dem`
- **Target:** `matarized/16_ode_integration/rk4/`
...
| `DYM` | `REAL` | NMAX | local | `DFMatrixKokkos<double>(NMAX)` |  |
| `DYT` | `REAL` | NMAX | local | `DFMatrixKokkos<double>(NMAX)` |  |
...
| `YOUT` | `REAL` | N | parameter (input) | `DFMatrixKokkos<double>(N)` |  |
| `YT` | `REAL` | NMAX | local | `DFMatrixKokkos<double>(NMAX)` |  |
...
```cpp
inline void rk4(DFMatrixKokkos<double>& y, DFMatrixKokkos<double>& dydx, int n, double x, double h, DFMatrixKokkos<double>& yout, double derivs)
```
...
2. **Translate routine** -- convert `RK4` to a C++ function as a `.hpp header`
...
project(rk4_matar_parallel CXX)
...
# --- Build the RK4 example ---
add_executable(rk4 main.cpp)
target_link_libraries(rk4 matar_lib)
target_include_directories(rk4 PRIVATE
...
```bash
cd fortran/16_ode_integration/rk4
...
```bash
cd matarized/16_ode_integration/rk4
mkdir -p build && cd build
cmake .. && make
./rk4 > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/16_ode_integration/rk4/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./rk4 > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./rk4 > omp4_output.txt 2>&1
...
| Field | Value |
|-------|-------|
| **Conversion order** | 22 of 202 |
| **Priority score** | 5 (reverse dependency count) |
| **Estimated effort** | medium (25 Fortran LOC, 3 dependencies) |
