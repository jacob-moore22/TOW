...
| **Purpose** | Finite difference equations for the sfroid example. |
| **Status** | `library_only` |
| **Complexity** | `medium` |
| **Fortran LOC** | 56 |
| **Subroutine** | `DIFEQ` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/17_boundary_value_problems/difeq/difeq.f` (56 lines)
- **Driver/demo:** _(none -- library-only or program-in-.f)_
- **Target:** `matarized/17_boundary_value_problems/difeq/`
...
|------|-------------|-------|------|-----------|-------|
| `INDEXV` | `INTEGER` | NYJ | parameter (input) | `DFMatrixKokkos<int>(NYJ)` |  |
| `IS1` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
| `ISF` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
...
```cpp
inline void difeq(int k, int k1, int k2, int jsf, int is1, int isf, DFMatrixKokkos<double>& indexv, int ne, DFMatrixKokkos<double>& s, int nsi, int nsj, DFMatrixKokkos<double>& y, int nyj, int nyk)
```
...
2. **Translate routine** -- convert `DIFEQ` to a C++ function as a `.hpp header`
...
project(difeq_matar_parallel CXX)
...
# --- Build the DIFEQ example ---
add_executable(difeq main.cpp)
target_link_libraries(difeq matar_lib)
...
```bash
cd matarized/17_boundary_value_problems/difeq
mkdir -p build && cd build
cmake .. && make
./difeq > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/17_boundary_value_problems/difeq/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./difeq > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./difeq > omp4_output.txt 2>&1
...
| Field | Value |
|-------|-------|
| **Conversion order** | 59 of 202 |
| **Priority score** | 1 (reverse dependency count) |
| **Estimated effort** | medium (56 Fortran LOC, 0 dependencies) |
