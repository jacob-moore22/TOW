...
| **Purpose** | Reduce columns of the banded matrix using results from pinvs. |
| **Status** | `library_only` |
| **Complexity** | `low` |
| **Fortran LOC** | 21 |
| **Subroutine** | `RED` (subroutine) |

## 2. Source Files

- **Fortran source:** `fortran/17_boundary_value_problems/red/red.f` (21 lines)
- **Driver/demo:** _(none -- library-only or program-in-.f)_
- **Target:** `matarized/17_boundary_value_problems/red/`
...
| `IC1` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
| `IZ1` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
| `IZ2` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
| `JC1` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
| `JCF` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
| `JM1` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
| `JM2` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
| `JMF` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
| `JZ1` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
| `JZ2` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
| `KC` | `INTEGER` | (scalar) | parameter (input) | `int` |  |
...
```cpp
inline void red(int iz1, int iz2, int jz1, int jz2, int jm1, int jm2, int jmf, int ic1, int jc1, int jcf, int kc, DFMatrixKokkos<double>& c, int nci, int ncj, int nck, DFMatrixKokkos<double>& s, int nsi, int nsj)
```
...
2. **Translate routine** -- convert `RED` to a C++ function as a `.hpp header`
...
project(red_matar_parallel CXX)
...
# --- Build the RED example ---
add_executable(red main.cpp)
target_link_libraries(red matar_lib)
...
```bash
cd matarized/17_boundary_value_problems/red
mkdir -p build && cd build
cmake .. && make
./red > serial_output.txt 2>&1
diff <(head -50 serial_output.txt) <(head -50 ../../../../fortran/17_boundary_value_problems/red/reference_output.txt)
```
...
OMP_NUM_THREADS=1 ./red > omp1_output.txt 2>&1
OMP_NUM_THREADS=4 ./red > omp4_output.txt 2>&1
